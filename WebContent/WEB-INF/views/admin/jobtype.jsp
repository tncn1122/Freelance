			...
			<section class="blog-posts-area section-gap">
				<div class="container">
					<div class="row">
						...
						<!-- GIỚI THIỆU CHUNG -->
							<div class="col-lg-9">
								<form:form modelAttribute="jobInfo" class="form-area contact-form" id="skillForm" action="" method="post">
                                    <dl class="row">
                                            <dt class="col-sm-3"><h5>Tên Lĩnh Vực:</h5></dt>
                                            <dd class="col-sm-9">
	                                            <spring:bind path="cagetory">
													<div class="${status.error ? 'has-error' : ''}">
														<!-- <form:select path="cagetory" />  -->
														<select  name = "cagetoryId">
															<c:forEach var="content" items="${cagetoryList}" varStatus="stt">
                                            ...
                                            <dt class="col-sm-3"><h5>Tên Loại Công Việc:</h5></dt>
                                            <dd class="col-sm-9">
	                                            <spring:bind path="congViec">
													<div class="${status.error ? 'has-error' : ''}">
														<form:input path="congViec" type="text" />
														<label for="congViec" placeholder="${status.error ? 'Vui Lòng Điền Loại Công Việc' : ''}"></label>
													</div>
												</spring:bind>
											</dd>
                                            <div class="mt-20 alert-msg" style="text-align: left;"></div>
											<div style = "margin: 0 auto;"><button name = "submit" class="primary-btn mt-20 text-white" style="float: right;">Thêm Loại Công Việc</button></div>
                                        </dl>
                                        <h5 style = "text-align: center">${message}</h5>
                                </form:form>	

							<br><br><br><br><br>
							<c:forEach var="content" items="${jobinfoList}" varStatus="stt">
								<div class="single-post d-flex flex-row formUserChaoGia " style = "padding: 30px">
									<div class="details col-lg-10">
										<div class="title d-flex flex-row justify-content-between">
											<div class="titles">
												<h3>${content.getCongViec()}</h3>
												<p>${content.getCagetory().getLinhVuc()}</p>
											</div>
										</div>
									</div>
	                                <div class = "thongTinChaoGia row">
	                                    <div style = "padding: 5px;"><a href="admin/job-type/${content.getId()}.htm" class="genric-btn success-border circle arrow">Sửa<span class="lnr lnr-arrow-right"></span></a></div>
	                                </div>
								</div>
							</c:forEach>
                            <!-- ================================================================= -->
